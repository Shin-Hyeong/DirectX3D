...
void GeometryHelper::CreateQuad(shared_ptr<Geometry<VertexTextureNormalData>> geometry)
{
	...
	// 사각형은 평면이기 때문에 Culling 설정을 생각해서 표면에서 90도인 단위 벡터(Normal)를 구함
	vtx[0].position = Vec3(-0.5f, -0.5f, 0.f);
	vtx[0].uv = Vec2(0.f, 1.f);
...
void GeometryHelper::CreateCube(shared_ptr<Geometry<VertexTextureNormalData>> geometry)
{
	...
	// 큐브의 6면 개별로 90도인 단위 벡터(Normal)를 구함
...
void GeometryHelper::CreateSphere(shared_ptr<Geometry<VertexTextureNormalData>> geometry, uint32 CircleCount, uint32 SliceCount)
{
	float radius = 0.5f; // 구의 반지름
	uint32 stackCount = CircleCount; // 가로 분할
	uint32 sliceCount = SliceCount; // 세로 분할
	...
	for (uint32 y = 1; y <= stackCount - 1; ++y)
	{
		...
		for (uint32 x = 0; x <= sliceCount; ++x)
		{
			...
			// 구의 중심점에서 임의의 접점으로 향하는 방향 벡터가 임의의 접점의 Normal이 됨.
			v.normal = v.position;
			// 구의 중심에서 정점으로 향하는 벡터
			v.normal.Normalize();
...
void GeometryHelper::CreateSphere(shared_ptr<Geometry<VertexTextureNormalTangentData>> geometry, uint32 CircleCount, uint32 SliceCount )
{
	float radius = 0.5f; // 구의 반지름
	uint32 stackCount = CircleCount; // 가로 분할
	uint32 sliceCount = SliceCount; // 세로 분할
