...
Matrix GetAnimationMatrix(VertexTextureNormalTangentBlend input)
{
    ...
    int animIndex = Keyframes.animIndex; // 현재 Animation 번호
    int currFrame = Keyframes.curreFrame; // 현재 재생중인 Frame
    int nextFrame = Keyframes.nextFrame; // 다음으로 재생할 Frame
    ...
    for (int i = 0; i < 4; i++)
    {
		...
		// Texture로 저장할때 하나의 행에 한 정점에 영향을 주는 Bone 4개의 데이터를 저장함
		// indices[i] : 해당 정점에 영향을 주는 Bone 4개중 i번째 bone index
		// * 4 : 한번에 4개씩 읽어서 한칸(4byte) * 4 = 16byte씩 읽도록 함
		// + i : 다음 데이터를 읽을 수 있도록 함
...
Matrix GetAnimationMatrix(VertexTextureNormalTangentBlend input)
{
    ...
    int currFrame = Keyframes.curreFrame;	// 현재 재생중인 Frame
    int nextFrame = Keyframes.nextFrame;	// 다음으로 재생할 Frame
    float ratio = Keyframes.ratio;			// 현재 Frame와 다음 Frame과의 비율(보간)
